...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, CoordinateLayer]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    # 5s    Model Summary: 283 layers, 7082421 parameters, 7082421 gradients, 16.4 GFLOPs
    # PP1.0 Model Summary: 367 layers, 3857195 parameters, 3857195 gradients, 8.2 GFLOPs
    parser = argparse.ArgumentParser()
    # parser.add_argument('--cfg', type=str, default='yolov5s_Coordinate.yaml', help='model.yaml')
    parser.add_argument('--cfg', type=str, default='yolov5_ConvNeXt.yaml', help='model.yaml')
    ...
    # flops, params = profile(model, (torch.rand(8 if torch.cuda.is_available() else 1, 3, 640, 640).to(device),))
